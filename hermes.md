...
以上两个文件在会话启动时，系统会将 [MEMORY.md](http://MEMORY.md)和 [USER.md](http://USER.md)的内容一次性读取，并格式化后注入到 System Prompt 的最前端。模型在处理后续对话时，无需重新计算这部分静态内存的 Token，从而大幅降低了首字响应时间（TTFT）并减少了算力成本。如果在会话中你修改了项目配置，Agent 会通过工具更新文件，但本次会话的 System Prompt 不会动态刷新。这种设计避免了频繁变动的 System Prompt 破坏 LLM 的缓存命中率。直到下一次新会话开启，Agent 才会加载最新的记忆。
...
你是一位拥有 10 年经验的资深后端工程师。你的核心目标是交付简单、可靠且可维护的解决方案。
...
Hermes 采用 SQLite 数据库与 JSONL 文件互补的存储方案，分别应对结构化查询与原始日志的需求：

* SQLite 数据库 (~/.hermes/state.db)：负责存储结构化元数据，包括 Session ID、来源平台（CLI/Telegram 等）、用户 ID、会话标题、模型配置、Token 消耗统计及完整消息历史。
* JSONL 转录文件 (~/.hermes/sessions/)：以纯文本形式按行存储原始对话流（Transcripts）。保留了每一次工具调用（Tool Calls）的原始请求与响应细节，主要用于审计与数据迁移。
...
2) 两个不同的会话不能共享相同的标题，标题最多1000个字符
...
#按照标题恢复session会话
hermes --resume  myfirst  等价于 hermes -r myfirest
```
...
执行如下命令删除指定会话，会话ID也需要通过执行“hermes sessions list”命令查看确定。
